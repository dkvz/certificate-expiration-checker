...
impl<'a> ProcessedCert<'a> {
        ...
        match get_certificate_expiry_date(path) {
            ...
            Err(err) => {
                desc = match err {
                    ...
                    CertReadError::CertParseError => {
                        String::from("Certificate data could not be parsed - Is this a public key?")
                    }
                    CertReadError::PEMError => {
                        String::from("The file appears not to be a PEM certificate")
                    }
                };
    ...
    pub fn is_alert_status(&self) -> bool {
        match self.status {
            CertStatus::Alert | CertStatus::Error | CertStatus::Expired => true,
            _ => false,
        }
    }
...
pub fn run(args: Vec<String>) -> Result<i32, String> {
    ...
    // optional.
    let opt_matches = opts.parse(args).map_err(|err| match err {
        Fail::ArgumentMissing(_) => usage(&opts),
        Fail::OptionDuplicated(dup) => format!("Please remove the duplicated option {}", dup),
    ...
    let filename = match opt_matches.opt_str("f") {
        ...
        None => {
            return Err(String::from(
                "Please provide a filename for the config file",
            ))
        }
    };
    ...
    let config = match ConfigFile::from(&filename) {
        ...
        Err(_) => {
            return Err(String::from(
                "Error reading the config file - Make sure it exists and is readable",
            ))
        }
    };
    ...
    if config.get_certificates().is_empty() {
        return Err(String::from(
            "The config file contained no certificate file paths to check",
        ));
    }
